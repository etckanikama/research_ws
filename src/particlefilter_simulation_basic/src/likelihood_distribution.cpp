...
int main(int argc, char **argv)
{
    ...
    {
        ROS_WARN("引数が指定されていません。デフォルト値として 8.0 を使用します。");
        param_global_init_x = 8.0;
    }
    // -----------------------------------------------------------------------------

    // 配置したいarrowの中心を受け取る（x,y,θ)
    double global_initial_x = param_global_init_x;
    double global_initial_y = 0.0;
    double global_initial_yaw = 0.0; // rad
    ...
    // // // パーティクルの座標の保存用csvファイル------------------------------------------------------
    std::stringstream ss;
    ss << "/home/hirayama-d/research_ws/src/particlefilter_simulation_basic/csv_distance/particle_coodinate_distribution_time_stamp_gazebo_down_sample_distance" << std::fixed << std::setprecision(1) << param_global_init_x << ".csv";
